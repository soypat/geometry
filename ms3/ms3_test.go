...
func TestRotationConversions(t *testing.T) {
	const tol = internal.Smallfloat32 / 10
	rotations := []Quat{
		Rotation(1, Vec{X: 1}),
...
func (rng *rngGen) Float() float32 {
	return float32(rng.rng.Float64())
}

func (rng *rngGen) FloatRange(start, end float32) float32 {
	return float32(rng.rng.Float64())*(end-start) + start
}
