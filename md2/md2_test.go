...
func TestLineDistanceInfinite2(t *testing.T) {
	...
	for _, ln := range lines {
		for _, p := range points {
			...
			// it accumulates more float64 rounding than the Hypot-based version.
...
func TestLineDistanceInfinite2Random(t *testing.T) {
	...
	randVec := func() Vec {
		return Vec{X: float64(rng.Float64()*20 - 10), Y: float64(rng.Float64()*20 - 10)}
	}
	for i := 0; i < 1000; i++ {
		a, b := randVec(), randVec()
...
func TestLineDistanceInfinite2OnLine(t *testing.T) {
	ln := Line{{1, 2}, {4, 8}}
	for _, ts := range []float64{-0.5, 0, 0.3, 1, 2.0} {
		p := ln.Interpolate(ts)
...
func bruteForceClosest(tri Triangle, p Vec) (best Vec, bestD2 float64) {
	const n = 4000
	bestD2 = math.MaxFloat32
	for j := 0; j < 3; j++ {
		edge := Line{tri[j], tri[(j+1)%3]}
		for i := 0; i <= n; i++ {
			q := edge.Interpolate(float64(i) / n)
...
func TestTriangleClosestPoint(t *testing.T) {
	...
	for _, tri := range tris {
		...
		for i := 0; i < 300; i++ {
			...
			p := Add(cen, Vec{X: float64(rng.Float64()*30 - 15), Y: float64(rng.Float64()*30 - 15)})
			if tri.Contains(p) {
				continue
			}
			c, side, vtx := tri.Closest(p)
